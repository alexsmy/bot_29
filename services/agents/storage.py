...
def _ensure_agent_filevault_folder() -> str:
    """Create 'Agents' folder in FileVault if missing, return its folder_id."""
...
def save_agent_response_to_filevault(
    ...
    """Save agent response to the Agents folder in FileVault."""
...
def save_agent_code_to_filevault(
    ...
    """Save uploaded agent code to the Agents folder in FileVault for visibility."""
    folder_id = _ensure_agent_filevault_folder()
    file_name = f"agent-code-{agent_name}.py"
    # Store the code as a regular file via filevault mechanism
    file_id = uuid4().hex
