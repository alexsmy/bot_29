...
async def send_admin_notification(message: str, setting_key: str, file_path: str = None):
    ...
    try:
        ...
        if not settings.get(setting_key, False):  # По умолчанию не отправляем, если ключ отсутствует
            return
        ...
        if file_path:
            ...
            await bot.send_message(chat_id=_admin_id, text=message, parse_mode='HTML')
