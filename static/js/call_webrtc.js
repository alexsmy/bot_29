...
async function probeIceServers(rtcConfig) {
callbacks.log('[PROBE] Starting ICE server probing...');
...
callbacks.log(`[PROBE] Probing complete. ${results.filter(r => r.status === 'Responded').length} servers responded.`);
return results;
...
export async function createPeerConnection(rtcConfig, localStream, selectedAudioOutId, connectionLogger, isCaller) {
...
if (isCaller) {
    const probeResults = await probeIceServers(rtcConfig);
...
await createPeerConnection(rtcConfig, localStream, callbacks.getSelectedAudioOutId(), connectionLogger, isCaller);
