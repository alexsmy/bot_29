...
async function loadConnections() {
    ...
    connectionsListContainer.innerHTML = sessions.map(session => {
        const callGroupsHtml = session.call_groups && session.call_groups.length > 0
            ? session.call_groups.map((call, groupIndex) => {
                
                const participantsHtml = call.participants && call.participants.length > 0 
                    ...
                    `).join('')
                    : '<p>Данные об участниках этого звонка отсутствуют.</p>';
